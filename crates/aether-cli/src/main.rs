...
async fn dispatch(cli: Cli, _cfg: EffectiveConfig) -> Result<()> {
    ...
    Commands::Deploy { dry_run, pack_only, compression_level, out, no_upload, no_cache, format } => { let _span = info_span!("cmd.deploy", dry_run, pack_only, compression_level, out=?out, no_upload, no_cache, format=?format); commands::deploy::handle(dry_run, pack_only, compression_level, out, no_upload, no_cache, format).await }

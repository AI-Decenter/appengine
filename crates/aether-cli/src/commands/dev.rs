...
pub async fn handle(hot: bool, interval: String) -> Result<()> {
    ...
    loop {
        ...
        if cur != last_digest {
            ...
            match deploy_handle(DeployOptions { dry_run:false, pack_only:true, compression_level:6, out:None, no_upload:false, no_cache:true, no_sbom:true, legacy_sbom:false, cyclonedx:false, format:None, use_legacy_upload:false, dev_hot:hot }).await {
                Ok(()) => { last_digest = cur; }

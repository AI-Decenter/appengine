...
async fn s3_presign_complete_flow() {
    ...
    sqlx::query("INSERT INTO applications (name) VALUES ($1) ON CONFLICT DO NOTHING").bind("s3app").execute(&pool).await.ok();
    let data = b"hello-s3-artifact".to_vec();
    let mut h = Sha256::new(); h.update(&data); let digest = format!("{:x}", h.finalize());
    // Presign
    let body = serde_json::json!({"app_name":"s3app","digest":digest}).to_string();
    ...
    let method = v["method"].as_str().unwrap(); assert_eq!(method, "PUT");
    // Perform PUT
    ...
    // Complete
    let comp_body = serde_json::json!({"app_name":"s3app","digest":digest,"size_bytes":data.len() as i64,"signature":null}).to_string();

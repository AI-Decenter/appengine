...
/// List deployments (optionally filter by app_name)
#[utoipa::path(get, path = "/deployments", params( ("app_name" = Option<String>, Query, description = "Filter by application name") ), responses( (status=200, body=[DeploymentItem]) ))]
...
pub async fn list_deployments(State(state): State<AppState>, Query(q): Query<DeploymentQuery>) -> ApiResult<Json<Vec<DeploymentItem>>> {
    ...
    let rows: Vec<Deployment> = if let Some(app_name) = q.app_name {
        sqlx::query_as::<_, Deployment>("SELECT d.id, d.app_id, d.artifact_url, d.status, d.created_at FROM deployments d JOIN applications a ON a.id = d.app_id WHERE a.name = $1 ORDER BY d.created_at DESC")
            .bind(app_name).fetch_all(pool).await.map_err(|e| ApiError::internal(format!("query error: {e}")))?
    } else {
